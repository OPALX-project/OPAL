...
{
  // Guard against invalid component indices generated by expression templates
  const unsigned c = e.Component;
  if (c < T::Size) {
    a[c] = b[c];
  }
}
...
{
  // Protect against out-of-range component access in vector-valued fields
  const unsigned c = e.Component;
  if (c < T::Size) {
    a[c] = b[c] * e.Slope[c] + e.Offset[c];
  }
}
