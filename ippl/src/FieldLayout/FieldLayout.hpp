...
template <unsigned Dim>
void FieldLayout<Dim>::initialize(const NDIndex<Dim> &domain, e_dim_tag *p, int vnodes) {
...
template <unsigned Dim>
void FieldLayout<Dim>::setup(const NDIndex<Dim> &domain, e_dim_tag *userflags, int vnodes) {
...
template <unsigned Dim>
FieldLayout<Dim>::FieldLayout(
    const NDIndex<Dim> &domain, const NDIndex<Dim> *idx_begin, const NDIndex<Dim> *idx_end)
    : Domain(domain) {
...
template <unsigned Dim>
FieldLayout<Dim>::FieldLayout(
    const NDIndex<Dim> &domain, const Vnode<Dim> *idx_begin, const Vnode<Dim> *idx_end)
    : Domain(domain) {
    // we have one more FieldLayout, indicate this
    // INCIPPLSTAT(incFieldLayouts);
    ...
    for (const Vnode<Dim> *p = idx_begin; p != idx_end; ++p) {
        typedef typename ac_id_vnodes::value_type v1;
        Local_ac.insert(
            v1(Unique::get(), new Vnode<Dim>((*p).getDomain(), mynode, (*p).getVnode())));
    }
    ...
    while ((--node_count) >= 0) {
        ...
        while (count-- > 0) {
            p.getMessage(*recv_mess);
            Vnode<Dim> *vnode = new Vnode<Dim>(p.getDomain(), other_node, p.getVnode());
            // For domains of extent 1 in all directions, must call
            // DomainMap::insert() with the noSplit flag set to true, to avoid an
            // assertion failure in Index::split. Also do this for domains of
            // extent ZERO. These are not supposed to happen in IPPL, but when the
...
//
template <unsigned Dim>
void FieldLayout<Dim>::Repartition(const NDIndex<Dim> *idxBegin, const NDIndex<Dim> *idxEnd) {
...
//
template <unsigned Dim>
void FieldLayout<Dim>::Repartition(const Vnode<Dim> *idxBegin, const Vnode<Dim> *idxEnd) {
    // Build a temporary FieldLayout to declare the temporary arrays.
    FieldLayout<Dim> tempLayout(Domain, idxBegin, idxEnd);
...
template <unsigned Dim>
void FieldLayout<Dim>::checkin(FieldLayoutUser &f, const GuardCellSizes<Dim> &gc) {
    checkinUser(f);
