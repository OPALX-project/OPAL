...
/*
                ...
                std::cout <<"Node " << Ippl::myNode() << " has ldomain " << lDomain_m << std::endl;
                std::cout <<"Node " << Ippl::myNode() << " has x slice (" << lDomain_m[0].first()*hr_m[0] << " , " <<  lDomain_m[0].last()*hr_m[0] << ")" << std::endl;
                std::cout <<"Node " << Ippl::myNode() << " has y slice (" << lDomain_m[1].first()*hr_m[1] << " , " <<  lDomain_m[1].last()*hr_m[0] << ")" << std::endl;
                std::cout <<"Node " << Ippl::myNode() << " has z slice (" << lDomain_m[2].first()*hr_m[2] << " , " <<  lDomain_m[2].last()*hr_m[0] << ")" << std::endl;
...
int main(int argc, char *argv[]){
        ...
        for (int it=0; it<iterations; it++) {
                ...
                if (test == TWOSTREAM) {
                        ...
                        P->interpolate_distribution((P->extend_r-P->extend_l)/(P->Nx),2.*P->Vmax/(P->Nv));
        ...
        if (test == SPHERE) {
                ...
                double Error, L2_exact, ErrorV,L2_V_exact, Total_E, Total_V, Total_E_exact, Total_V_exact, Potential_energy;
                ...
                if(Ippl::myNode()==0) {
                        ...
                        ofs << nr[0] << "," << P->getTotalNum() << "," << interaction_radius << "," << eps << "," << alpha << "," << Error << "," << Relative_error << "," << fabs(Total_E-Total_E_exact)/Total_E_exact <<  "," << ErrorV << "," << Relative_V_error << "," << fabs(Total_V-Total_V_exact)/Total_V_exact << "," << std::abs(Potential_energy-U)/U << std::endl;
                        ofs.close();
                }
        }
