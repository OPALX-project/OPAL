...
void CSRIGFWakeFunction::calculateGreenFunction(PartBunchBase<double, 3>* bunch,
    ...
    for (unsigned int i = 0; i < numOfSlices; ++i) {
        ...
        double b = std::sqrt(xmu * xmu + 1.0) + xmu;
        if (xmu < 1e-3)
            Chi_m[i] = chi_const + 0.5 * std::pow(xmu, 2) - 7.0 / 54.0 * std::pow(xmu, 4) + 140.0 / 2187.0 * std::pow(xmu, 6);
        else
            Chi_m[i] = 9.0 / 16.0 * (3.0 * (-2.0 * xmu * std::pow(b, 1.0/3.0) + std::pow(b, 2.0/3.0) + std::pow(b, 4.0/3.0)) +
                                     std::log(std::pow((1 - std::pow(b, 2.0 / 3.0)) / xmu, 2) / (1 + std::pow(b, 2.0 / 3.0) + std::pow(b, 4.0 / 3.0))));
    }
