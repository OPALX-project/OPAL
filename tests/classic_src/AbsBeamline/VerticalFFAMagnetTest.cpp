...
TEST_F(VerticalFFAMagnetTest, VectorPotentialTest) {
    // check the curl of the vector potential is the field 
    double x = length_m*0.1;
    double y = length_m*0.05;
    double z = length_m*0.8;
    VerticalFFAMagnet* magnet = dynamic_cast<VerticalFFAMagnet*>(magnet_m->clone());
    ...
    MaxwellTest maxTest(Vector_t(dr, dr, dr), 1., magnet);
    maxTest.printHeading(std::cerr);
    std::vector<double> tol({10., 0.5, 0.5, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05});
    std::cerr << "Testing Vector Potential" << std::endl;
    std::vector<std::string> ax({"x", "y", "z"});
    for (size_t i = 1; i < 8; i += 1) {
        magnet->setMaxOrder(i);
        magnet->initialise();
        Vector_t pos(x*mm, y*mm, z*mm);
        Vector_t B = maxTest.getB(pos, 0);
        Vector_t curlA = maxTest.curlA(pos, 0.);
        EXPECT_NEAR(curlA[0], B[0], tol[i-1]);
        EXPECT_NEAR(curlA[1], B[1], 1e-9);
        EXPECT_NEAR(curlA[2], B[2], 1e-9);
        std::cerr << "Max Order: " << i << std::endl;
        for (size_t j = 0; j < 3; ++j) {
            std::cerr << "   " << ax[j] << " curlA: " << curlA[j] 
                      << " B: " << B[j] << " diff: " << curlA[j]-B[j]
                      << std::endl;
        }
    }
}
